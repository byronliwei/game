...
class IndexAction extends CommonAction {
    ...
    public function lanIndex(){
        ...
        if($country_code=="US" && strstr($_SERVER['HTTP_HOST'],'www')){
            $this->assign('country','US');
        }
        $version=$_GET['version'];
        // echo 'mobile';die;
    ...
    public function gameList($limit='',$page='',$_cata){
        ...
        if(is_array($relid)){
            ...
            if($cata == "Hot-games" ||$cata == "New-games" ){
                ...
                $where = [];
                $where['p.'.$w] = array('eq',1);
                $where['q.swf'] = array('not like','%swf%');
//                $where = 'p.'.$w.' = 1';
            }else{
                $where = [];
                $where['p.rel_id'] = array('eq',$relid);
                $where['q.seo_name'] = array('neq',"");
//                $where = 'p.rel_id='.$relid.' AND q.seo_name!=""';
                $where['q.swf'] = array('notlike','%swf%');
            }
        ...
        foreach ($list as $key => $value) {
            ...
            $list[$key]['img'] = $value['img'].'?page=index&website=gamev6&type=mobile&gid='.$value['game_id'];
            if($_sort=='webgame'){
                if($_3d_type=='direct'){
                    $swf=json_decode($value['swf'],true);
    ...
    public function randomGame($num='',$where=''){
        ...
        shuffle($list);
        foreach ($list as $key => $value) {
            $list[$key]['img'] = $value['img'].'?page=play&website=gamev6&type=mobile&gid='.$value['id'];
        }
        return $list;
    }
...
//dump($uriArr);exit;
    ...
    public function indexLoad($limit,$page){
        if($page){
            $limit=$limit*($page-1).','.$limit;
        }
        $Tb=M('top100_list','game_');
