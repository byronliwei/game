		...
		if (!h.noshare) {
			if (null == f) {
				...
				c.on("click", function (c) {
					 ...
					 window.open(mobConfig.sahrepage)
				})
			}
			a.stage.addChild(f)
		}
	...
	a.showFollowAnim = function (c) {
		if (!h.followed) {
			if (null == g) {
				...
				g.name = "follow";
				g.on("click", function () {
					IS_TOUCH && event.nativeEvent instanceof MouseEvent || window.open(mobConfig.mainpage)
				})
			} else
...
function MyProgressBar(a, d, b) {
	...
	b.textAlign = "center";
	this.addChild(b);
	b = new createjs.Text("Cat Yoga", "bold 38px Arial", "black");
	b.x = a / 2;
	b.y = 240;
	b.textAlign = "center";
	this.addChild(b);
	b = new createjs.Text("thehotgames.com", "30px Arial", "black");
	b.x = a / 2;
	b.y = d - 80;
	b.textAlign = "center";
	this.addChild(b)
