	...
	(function(){
		...
		if (!chrome.app.isInstalled&&isChrome&&!localStorage.installCheck&&site_config.cata=='PlayPage') {
			...
			var noticeLang={
				...
				'es':['Su versi\u00f3n de Flash Plugin es demasiado baja, por favor, actualice!','Actualizar Ahora'],
			...
			headerWarn.show({
				height:37,
				string:'<style>.show-close-btn:hover{background:rgb(242, 173, 173);border-radius:15px;color:#fff!important;box-shadow: inset 0px 1px 1px rgba(51, 51, 51, 0.15);text-shadow: 0px 1px 1px rgba(51, 51, 51, 0.29);}</style><div id="warnbox" style="color:#252525;position:relative;background-image:-webkit-gradient(linear,0% 0%, 0% 100%, from(#FDEDA6), to(#FBE793), color-stop(0.5,#FCE99C));height:36px;line-height:36px;font-size:16px;border-bottom:solid 1px #AAAAAB"><img src="/Public/static/images/icon_notice.png" style="vertical-align: -6px;width:24px;margin: 0px 15px;">'+noticeStr[0]+'<button onclick="installExt();" style="margin-left:20px;padding:4px 10px;font-size: 14px;"> '+noticeStr[1]+' </button><div class="show-close-btn" style="position: absolute;right: 10px;top: 8px;height: 22px;width: 22px;text-align: center;line-height: 22px;font-size: 18px;color: #666;cursor:default;">&times;</div></div>'
			});

			$('#warnbox').live('click',function(){
				installExt();
			})

			$('.show-close-btn').live('click',function(e){
