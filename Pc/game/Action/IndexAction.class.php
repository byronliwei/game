...
class IndexAction extends CommonAction {
    ...
    public function index(){
        ...
        header('Location:http://'.$_SERVER['HTTP_HOST'].'/'.$lan.$param);
        exit;
    ...
    public function gameList($limit='',$page='',$_cata){
        ...
        $cata=$_REQUEST['cata'];
        $gamecates = explode('-',$cata);
        $gamecate = $gamecates[0];
        if($_cata){
            $cata=$_cata;
        }
        ...
        if($cata){
            if($cata == "Hot-games" ||$cata == "New-games" ){
                $Tb=M('game_release_list');
                ...
                $cata = $catas[0];
                $CA = M('game_category');
                $relids = $CA->where('name ="'.$cata.'"')->field('release_id')->find();
                $relid = explode(',',$relids['release_id']);
            }
            // $Tb=M('game_release_list');
            // $vo=$Tb->where('release_alias="'.$cata.'"')->limit(1)->select();
            // $relid=$vo[0]['id'];
        ...
        if(is_array($relid)){
            $where = array(
                'p.rel_id' => array('in',$relid),
                'q.seo_name' => array('neq','')
            );
            $order = 'p.order_v6 DESC,p.id DESC';
        }else{
            if($cata == "Hot-games" ||$cata == "New-games" ){
                if($cata == "Hot-games")$w = 'hot';
                if($cata == "New-games")$w = 'new';
                $where = 'p.'.$w.' = 1';
                $order = 'p.order_v6_pc DESC,p.id DESC';
            }else{
        ...
        foreach ($list as $key => $value) {
            ...
            $list[$key]['img'] = $value['img'].'?page=index&website=gamev6&type=pc&gid='.$value['game_id'];
        }
    ...
    private function tagGameList($limit='',$page=''){
        ...
        ->alias('p')
        ->join('game_list AS q ON p.game_id=q.id')
        ->field('q.name,q.img,q.seo_name,q.sort')
    ...
    public function readLang(){
        ...
        // 分类排序
        $Tb = M('game_category');
        $vo = $Tb->order('sort_v6 desc')->select();
        // dump($vo);die;
        // dump($lan);die;
        $this->assign('lan',$lan);
        $this->assign('cate',$vo);
    ...
    /************###**********/
    public function play($tpl='',$relateNum=14){
        $name=(string)$_GET['n'];
        ...
        // dump($data);die;
        // $a = $this->randomGame($relateNum,$where);
        // dump($a);die;
        $gameList = $this->randomGame($relateNum,$where);
        
        foreach ($gameList as $key => $value) {
            if ($tpl == 'infoPage') {
                $gameList[$key]['img'] = $value['img'].'?page=info&website=gamev6&type=pc&gid='.$value['id'];
            }else{
                $gameList[$key]['img'] = $value['img'].'?page=play&website=gamev6&type=pc&gid='.$value['id'];
            }
        }
        $this->assign('gameList',$gameList);
        $this->randomGame($relateNum,$where);
    
        $this->assign('gameList1',$this->topGame());
        $this->assign('gameDetail',$game);
    ...
    public function randomGame($num='',$where=''){
        ...
        $rand=rand(0, $len-$num);
        $list=$Tb   
        ->order('id DESC')->where($where)->limit($rand>=0?$rand:0,$rand+$num)->order("id desc")->select();
...
//dump($uriArr);exit;
    ...
    public function topGame(){
        ...
        $redis->connect('127.0.0.1', 6379);
      	$list = json_decode($redis->get('gamev6top'));
        $Tb=M('list','game_');
        $w = array(
            'id'=>array('in',$list)
        );
        $Top = $Tb->where($w)->select();

        foreach ($Top as $key => $value) {
            $Top[$key]['img'] = $value['img'].'?page=info&website=gamev6&type=pc&gid='.$value['id'];
        }
        return $Top;
    }

    // public function getTagList(){
    //     $model = M('game_release_list');
    //     $list = $model->where('lang = "" AND status = 1')->select();
    //     foreach ($list as $key => &$value) {
    //         $v = explode('-',$value['release_alias']);
    //         $value['name'] = $v[0];
    //         $v = null;
    //     }
    //     dump($list);die;
